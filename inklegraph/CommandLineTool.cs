...
namespace Ink
{
	class CommandLineTool : Ink.IFileHandler
	{
        ...
        {
            Console.WriteLine (
                "Usage: inklegraph <options> <ink file> \n"+
		...
		CommandLineTool(string[] args)
		{
            ...
            if (opts.outputFile == null)
                opts.outputFile = Path.ChangeExtension (opts.inputFile, ".ink.graph.json");
            ...
            var grapher = new CommandLineGrapher(story, compiler);

            try {
                var jsonObject = new Dictionary<string, object>();
                grapher.Render (jsonObject);
